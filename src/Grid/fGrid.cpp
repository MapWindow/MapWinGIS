...
///\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/
	...
	void fGrid::binaryWriteHeader( FILE * out )
	{
		...
		{	int size_of_pad = MAX_STRING_LENGTH - static_cast<int>(strlen(projection));
		...
		{	int size_of_pad = MAX_STRING_LENGTH - static_cast<int>(strlen(notes));
			char * pad = new char[size_of_pad];
			for( int p = 0; p < size_of_pad; p++ )
				pad[p] = 0;
			fwrite(pad, sizeof(char), size_of_pad, out );
	...
	{	
		...
		if( grid_layer >= 0 )
		{
			...
			{	
				...
				register float *buf = static_cast<float*>(row_buf1);
				for( int i = 0; i < gridHeader.getNumberCols(); i++)
				{	
					if( buf[i] == float_null )
						data[j][i] = nodata;
...
/*			for ( int j = 0; j < gridHeader.getNumberRows(); j++)
	...
	bool fGrid::esriWriteMemoryToDisk( void(*callback)(int number, const char * message ) )
	{
		...
		for( int j = 0; j < gridHeader.getNumberRows(); j++)
		{
			...
			putwindowrowfloat( grid_layer, j, static_cast<float*>(row_buf1));
	...
	float fGrid::esriGetValueDisk( int Column, int Row )
	{
		...
		if( Row == current_row - 1 )
		{	
			if( static_cast<float*>(row_buf1)[Column] == float_null )
				return gridHeader.getNodataValue();
			return static_cast<float>(static_cast<float*>(row_buf1)[Column]);
		}
		else if( Row == current_row )
		{
			if( static_cast<float*>(row_buf2)[Column] == float_null )
				return gridHeader.getNodataValue();
			return float((static_cast<float*>(row_buf2)[Column]));
		}
		else if( Row == current_row + 1 )
		{
			if( static_cast<float*>(row_buf3)[Column] == float_null )
				return gridHeader.getNodataValue();
			return static_cast<float>(static_cast<float*>(row_buf3)[Column]);
		}
		else
		{	esriBufferRows( Row );
			if( static_cast<float*>(row_buf2)[Column] == float_null )
				return gridHeader.getNodataValue();
			return static_cast<float>(static_cast<float*>(row_buf2)[Column]);
		}
	...
	void fGrid::esriSetValueDisk( int Column, int Row, float Value )
	{
		...
		else if( Row == current_row )
		{
			...
			register float *buf = static_cast<float*>(row_buf2);
			buf[Column] = value;
		...
		else if( Row == current_row + 1 )
		{
			...
			register float *buf = static_cast<float*>(row_buf3);
			buf[Column] = value;
	...
	void fGrid::esriBufferRows( int center_row )
	{
		...
		{	register float *ibuf;
			...
			for( int i = 0; i < gridHeader.getNumberCols(); i++ )
			{	
				ibuf = static_cast<float*>(row_buf1);
				ibuf[i] = float_null;
				ibuf = static_cast<float*>(row_buf2);
				ibuf[i] = float_null;
				ibuf = static_cast<float*>(row_buf3);
				ibuf[i] = float_null;
			}
			return;
		}
	...
	bool fGrid::esriSaveAs( CString filename, void(*callback)(int number, const char * message) )
	{
		...
		float nodata = gridHeader.getNodataValue();
		register float *buf = static_cast<float*>(temp_row_buf);
		for( int j = 0; j < gridHeader.getNumberRows(); j++)
		{
			for( int i = 0; i < gridHeader.getNumberCols(); i++)
			{
...
///\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/
		...
		for( int row = 0; row < gridHeader.getNumberRows(); row++ )
		{
			for( int column = 0; column < gridHeader.getNumberCols(); column++ )
			{
				...
				if( value == -255 )
				{
					...
					setValue( column, row, static_cast<float>(average) );
				}
			}
		}
	...
	{	
		...
		h.setNodataValue( static_cast<float>(voidvalue) );
 	...
 	void fGrid::baseAndId()
	{
		int len,j;
		//parse out base_name 
		len = static_cast<int>(strlen(file_name));
		for(j=0;j<len-8;j++)
				...
				*/
				...
				if(li != nodata_value && li != fillvalue) 
				{
					...
					if( li == 32767 || li == -32767 )
						...
						setValue( i, j, static_cast<float>(li) );
