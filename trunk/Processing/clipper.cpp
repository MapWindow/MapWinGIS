...
* http://www.me.berkeley.edu/~mcmains/pubs/DAC05OffseClipper::Polygon.pdf              *
*                                                                              *
*******************************************************************************/
...
bool ClipperBase::PreparePolygons(const Polygons &ppg, PolyType polyType, std::vector<TEdge*>& resultEdges)
{
	resultEdges.clear();
	for (Polygons::size_type i = 0; i < ppg.size(); ++i) {
		TEdge* edges = PreparePolygon(ppg[i], polyType);
		if (edges) {
			resultEdges.push_back(edges);
		}
	}
	return resultEdges.size() > 0;
}

TEdge* ClipperBase::PreparePolygon(const Polygon &pg, PolyType polyType)
{
	int len = (int)pg.size();
...
bool ClipperBase::AddPreparedPolygons( std::vector<TEdge*>& edges ) {
	for (size_t i = 0; i < edges.size(); i++) {
		this->AddPreparedPolygon(edges[i]);
	}
	return true;
}

bool ClipperBase::AddPreparedPolygon( TEdge *edges )
{
	if (edges) {
...
//------------------------------------------------------------------------------
// OffseClipper::Polygon functions ...
//------------------------------------------------------------------------------
