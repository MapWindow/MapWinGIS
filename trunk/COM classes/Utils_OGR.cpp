...
/* -------------------------------------------------------------------- */
	...
	for (int iArg = 1; iArg < nArgc; iArg++)
	{
		...
		else if( EQUAL(sArr[iArg],"-f") && iArg < nArgc-1 )
        {
            bFormatExplicitelySet = TRUE;
            pszFormat = sArr[++iArg];
        }
        else if( EQUAL(sArr[iArg],"-dsco") && iArg < nArgc-1 )
        {
            papszDSCO = CSLAddString(papszDSCO, sArr[++iArg] );
        }
        else if( EQUAL(sArr[iArg],"-lco") && iArg < nArgc-1 )
        {
            papszLCO = CSLAddString(papszLCO, sArr[++iArg] );
        }
        else if( EQUAL(sArr[iArg],"-preserve_fid") )
        {
            bPreserveFID = TRUE;
        }
        else if( EQUALN(sArr[iArg],"-skip",5) )
        {
            bSkipFailures = TRUE;
            nGroupTransactions = 1; /* #2409 */
        }
        else if( EQUAL(sArr[iArg],"-append") )
        {
            bAppend = TRUE;
            bUpdate = TRUE;
        }
        ...
        else if( EQUAL(sArr[iArg],"-update") )
        {
            bUpdate = TRUE;
        }
        else if( EQUAL(sArr[iArg],"-fid") && iArg < nArgc-1 )
        {
            nFIDToFetch = atoi(sArr[++iArg]);
        }
        ...
        else if( EQUAL(sArr[iArg],"-nln") && iArg < nArgc-1 )
        {
            pszNewLayerName = CPLStrdup(sArr[++iArg]);
        }
        else if( EQUAL(sArr[iArg],"-nlt") && iArg < nArgc-1 )
        {
            int bIs3D = FALSE;
            CPLString osGeomName = sArr[iArg+1];
            if (strlen(sArr[iArg+1]) > 3 &&
                EQUALN(sArr[iArg+1].GetBuffer(0) + strlen(sArr[iArg+1]) - 3, "25D", 3))
            {
                bIs3D = TRUE;
        ...
        else if( (EQUAL(sArr[iArg],"-tg") ||
                  EQUAL(sArr[iArg],"-gt")) && iArg < nArgc-1 )
        {
            nGroupTransactions = atoi(sArr[++iArg]);
        }
        else if( EQUAL(sArr[iArg],"-s_srs") && iArg < nArgc-1 )
        {
            pszSourceSRSDef = sArr[++iArg];
        }
        else if( EQUAL(sArr[iArg],"-a_srs") && iArg < nArgc-1 )
        {
            pszOutputSRSDef = sArr[++iArg];
        ...
        else if( EQUAL(sArr[iArg],"-t_srs") && iArg < nArgc-1 )
        {
            pszOutputSRSDef = sArr[++iArg];
            bTransform = TRUE;
        }
        ...
        else if( EQUAL(sArr[iArg],"-select") && iArg < nArgc-1 )
        {
            pszSelect = sArr[++iArg];
            papszSelFields = CSLTokenizeStringComplex(pszSelect, " ,", 
                                                      FALSE, FALSE );
        }
        else if( EQUAL(sArr[iArg],"-segmentize") && iArg < nArgc-1 )
        {
            eGeomOp = SEGMENTIZE;
            dfGeomOpParam = atof(sArr[++iArg]);
        }
        else if( EQUAL(sArr[iArg],"-simplify") && iArg < nArgc-1 )
        {
            eGeomOp = SIMPLIFY_PRESERVE_TOPOLOGY;
            dfGeomOpParam = atof(sArr[++iArg]);
        }
        else if( EQUAL(sArr[iArg],"-fieldTypeToString") && iArg < nArgc-1 )
        {
            papszFieldTypesToString =
                    CSLTokenizeStringComplex(sArr[++iArg], " ,", 
                                             FALSE, FALSE );
        ...
        else if( EQUAL(sArr[iArg],"-clipsrc") && iArg < nArgc-1 )
        {
            VSIStatBufL  sStat;
            bClipSrc = TRUE;
            if ( iArg < nArgc-4 && IsNumber(sArr[iArg+1]) )
            {
                OGRLinearRing  oRing;
            ...
            else if ((EQUALN(sArr[iArg+1], "POLYGON", 7) ||
                      EQUALN(sArr[iArg+1], "MULTIPOLYGON", 12)) &&
                      VSIStatL(sArr[iArg+1], &sStat) != 0)
            {
                char* pszTmp = (char*) sArr[iArg+1].GetBuffer(0);
            ...
            else if (EQUAL(sArr[iArg+1], "spat_extent") )
            {
                iArg ++;
            }
            else
            {
                pszClipSrcDS = sArr[iArg+1];
                iArg ++;
            }
        }
        else if( EQUAL(sArr[iArg],"-clipsrcsql") && iArg < nArgc-1 )
        {
            pszClipSrcSQL = sArr[iArg+1];
            iArg ++;
        }
        else if( EQUAL(sArr[iArg],"-clipsrclayer") && iArg < nArgc-1 )
        {
            pszClipSrcLayer = sArr[iArg+1];
            iArg ++;
        }
        else if( EQUAL(sArr[iArg],"-clipsrcwhere") && iArg < nArgc-1 )
        {
            pszClipSrcWhere = sArr[iArg+1];
            iArg ++;
        }
        else if( EQUAL(sArr[iArg],"-clipdst") && iArg < nArgc-1 )
        {
            VSIStatBufL  sStat;
            if ( iArg < nArgc-4 && IsNumber(sArr[iArg+1]) )
            {
            ...
            {
                pszClipDstDS = sArr[iArg+1];
                iArg ++;
            }
        }
        else if( EQUAL(sArr[iArg],"-clipdstsql") && iArg < nArgc-1 )
        {
            pszClipDstSQL = sArr[iArg+1];
            iArg ++;
        }
        else if( EQUAL(sArr[iArg],"-clipdstlayer") && iArg < nArgc-1 )
        {
            pszClipDstLayer = sArr[iArg+1];
            iArg ++;
        }
        else if( EQUAL(sArr[iArg],"-clipdstwhere") && iArg < nArgc-1 )
        {
            pszClipDstWhere = sArr[iArg+1];
            iArg ++;
        }
        else if( EQUAL(sArr[iArg],"-splitlistfields") )
        {
            bSplitListFields = TRUE;
        }
        else if ( EQUAL(sArr[iArg],"-maxsubfields") && iArg < nArgc-1 )
        {
            if (IsNumber(sArr[iArg+1]))
            {
                int nTemp = atoi(sArr[iArg+1]);
                if (nTemp > 0)
                {
        ...
        else if( EQUAL(sArr[iArg],"-explodecollections") )
        {
            bExplodeCollections = TRUE;
        }
        else if( EQUAL(sArr[iArg],"-zfield") && iArg < nArgc-1 )
        {
            pszZField = sArr[iArg+1];
            iArg ++;
        }
	}
