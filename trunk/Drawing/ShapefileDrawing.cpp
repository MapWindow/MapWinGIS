...
#pragma region DrawPolyCategoryGdiPlus
// *************************************************************
//		DrawPolygonCategory()
// *************************************************************
// Polygon or polyline category
void CShapefileDrawer::DrawPolyCategoryGDIPlus( CDrawingOptionsEx* options, std::vector<int>* indices, bool drawSelection)
{
	...
	// for vdmGDIMixed we'll use GDI for drawing outlines 
	// but only if width is equal to 1.  If it's not we switch to vdmGDIPlus
	tkVectorDrawingMode drawingMode = options->drawingMode;
	if ( options->drawingMode == vdmGDIMixed &&
		 (options->lineWidth != 1  ||
		 !options->linesVisible) ) 	 	
	{
	...
	// ----------------------------------------------------
	//	 We have to draw per shape gradient and initialize 
	//  GDIPlus::Brush on each redraw
	// ----------------------------------------------------
	VARIANT_BOOL fastMode;
	_shapefile->get_FastMode(&fastMode);
	
	if ( options->fillVisible &&
		 options->fillType == ftGradient && 
		 options->fillGradientBounds == gbPerShape &&
		 _shptype == SHP_POLYGON )
	{
		for (int j = 0; j < (int)indices->size(); j++)
		{
			GraphicsPath pathFill(Gdiplus::FillModeWinding);
		
			if (DrawPolygonGDIPlus((*indices)[j], pathFill , delta, pointColor, drawingMode, xMin, xMax, yMin, yMax))
			{
				// drawing fill
		...
		if ( options->linesVisible && drawingMode == vdmGDIMixed)
		{
			_dc->EndPath();		
			_dc->StrokePath();
		}
	...
	else
	{
		...
		for (int j = 0; j < (int)indices->size(); j++)
		{
			DrawPolygonGDIPlus((*indices)[j], *path, delta, pointColor, drawingMode, xMin, xMax, yMin, yMax);
		}
		...
		if (_fastMode)
		{
			...
			if (shpData)
			{
				...
				if (WithinVisibleExtents(xMin, xMax, yMin, yMax))
				{
					...
					{
						AddPolygonToPath(&path, shpData, drawingMode);
						return true;
					}
					else
					{
						DrawPolygonPointGDI(xMin, xMax, yMin, yMax, pointColor);
						return false;
					}
				}
			}
			...
			if (data)
			{
				...
				if (WithinVisibleExtents(xMin, xMax, yMin, yMax))
				{
					...
					if ((xMax - xMin >= minSize) || (yMax - yMin >= minSize))	// the poly must be larger than a pixel at a current to be drawn
					{
						...
						AddPolygonToPath(&path, shapeData, drawingMode);
						delete shapeData;
						result = true;
					}
					else
...
// ****************************************************************
void CShapefileDrawer::DrawPolyCategoryGDI(CDrawingOptionsEx* options, std::vector<int>* indices, bool drawSelection)
{
	IShapeWrapper* shp = NULL;
			...
			if (fastMode)
			{
				...
				if (shpData)
				{
					...
					if (WithinVisibleExtents(xMin, xMax, yMin, yMax))
					{
						...
						{
							DrawPolyGDI( shpData, options, *path, options->verticesVisible?true:false);
						}
						else
						{
							DrawPolygonPointGDI(xMin, xMax, yMin, yMax, pointColor);
			...
			else
			{
				...
				if (data)
				{
					...
					if (WithinVisibleExtents(xMin, xMax, yMin, yMax))
					{
						...
						{
							PolygonData* shapeData = _sfReader->ReadPolygonData(data);
							DrawPolyGDI( shapeData, options, *path, options->verticesVisible?true:false);
							delete shapeData;
						}
						else
						{
...
//		DrawPolyline()
// ******************************************************************
void CShapefileDrawer::DrawPolylinePatternCategory(CDrawingOptionsEx* options, std::vector<int>* indices, bool drawSelection)
{
	...
	for (int j = 0; j < (int)indices->size(); j++)
	{
		DrawPolygonGDIPlus((*indices)[j], *path, delta, pointColor, vdmGDIPlus, xMin, xMax, yMin, yMax);
	}
	
	// the drawing
	DrawPolylinePath(path, options, drawSelection);
...
inline void CShapefileDrawer::DrawPolygonPointGDI(double &xMin, double& xMax, double& yMin, double& yMax, OLE_COLOR& pointColor)
{
