...
CPLXMLNode* CLabelOptions::SerializeToTree(CString elementName )
{
	...
	if (opt->offsetX != this->offsetX)
		Utility::CPLCreateXMLAttributeAndValue( psTree, "OffsetX", CPLString().Printf("%d", (int)this->offsetX ));
	
	if (opt->offsetY != this->offsetY)
		Utility::CPLCreateXMLAttributeAndValue( psTree, "OffsetY", CPLString().Printf("%d", (int)this->offsetY ));
