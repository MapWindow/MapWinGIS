...
// 		Constructor										
// *********************************************************************
CLabelDrawer::CLabelDrawer(CDC* dc, Extent* extents, double pixelPerProjectionX, double pixelPerProjectionY, double currentScale, 
...
CLabelDrawer::CLabelDrawer(CDC* dc, Extent* extents, CCollisionList* collisionList, double mapRotation)
{
	_extents = extents;
...
void CLabelDrawer::DrawLabels( ILabels* LabelsClass )
{
	...
	Gdiplus::CompositingQuality compositingQualityInit = graphics.GetCompositingQuality();
	Gdiplus::SmoothingMode smoothingModeInit = graphics.GetSmoothingMode();
	Gdiplus::TextRenderingHint textRenderingHintInit = graphics.GetTextRenderingHint();
	...
	graphics.SetCompositingQuality((CompositingQuality)compositingQuality);
	graphics.SetSmoothingMode((SmoothingMode)smoothingMode);
	graphics.SetTextRenderingHint((TextRenderingHint)textRenderingHint);
    graphics.SetTextContrast(0u);
	...
	for (int iCategory = numCategories - 1; iCategory >= -1; iCategory--)
	{
		...
		if (useGdiPlus)
		{
			...
			AlignmentToGDIPlus(m_options->inboxAlignment, gpStringFormat);
		...
		for (unsigned int k = 0; k < size; k++ )
		{
			...
			for (int j =0; j < (int)parts->size(); j++ )
			{
				...
				else
				{
					...
					graphics.MeasureString(wText, lbl->text.GetLength(), gpFont, PointF(0.0f, 0.0f), &gpRect);
				...
				else
				{
					...
					if (m_options->frameTransparency != 0 && m_options->frameVisible)
					{
						if (m_options->frameGradientMode != gmNone)
						{
							...
							this->DrawLabelFrameGdiPlus(graphics, gpBrushFrameGrad, *gpPenFrameOutline, gpRect, m_options);
							delete gpBrushFrameGrad;
						}
						else
						{
							this->DrawLabelFrameGdiPlus(graphics, gpBrushFrame, *gpPenFrameOutline, gpRect, m_options);
						}
					}
					...
					if (m_options->fontTransparency != 0)
					{
						...
						if (m_options->shadowVisible)
						{
							...
							gp->Transform(&mtx1);
							graphics.FillPath(gpBrushShadow, gp);
						...
						if  (m_options->haloVisible && m_options)
							graphics.DrawPath(gpPenHalo, gp);

						if(m_options->fontOutlineVisible)
							graphics.DrawPath(gpPenFontOutline, gp);
						...
						if (m_options->fontGradientMode != gmNone)
						{
							...
							graphics.DrawString(wText, wcslen(wText), gpFont, gpRect, &gpStringFormat, gpBrushFontGrad);	// TODO: we need speed test here to choose the function
							delete gpBrushFontGrad;
						}
						else
						{
							graphics.DrawString(wText, wcslen(wText), gpFont, gpRect, &gpStringFormat, gpBrushFont);
						}
		...
		// we use the same brushFrameBack; we have several pens and select them after each use
	...
	graphics.SetCompositingQuality(compositingQualityInit);
	graphics.SetSmoothingMode(smoothingModeInit);
	graphics.SetTextRenderingHint(textRenderingHintInit);
