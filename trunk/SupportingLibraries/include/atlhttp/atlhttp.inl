			...
			) throw(...)
{
	...
	if(!AtlEscapeUrl(szUrl,szTmp,0,ATL_URL_MAX_URL_LENGTH-1,ATL_URL_NO_ENCODE))			// encode more is changed from the default setting
		return false;
...
inline bool CAtlHttpClientT<TSocketClass>::ReadChunkedBody() throw()
{
	...
	int length = result_buffer.GetLength();
	if (!m_current.Append((LPCSTR)result_buffer, length))
		return false;
