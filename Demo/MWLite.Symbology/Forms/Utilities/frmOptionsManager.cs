...
namespace MWLite.Symbology.Forms.Utilities
{
    public partial class frmOptionsManager : Form
    {
            ...
            if (form.ShowDialog(this) == DialogResult.OK)
            {
                ...
                if (System.IO.File.Exists(name))
                {
                    if (MessageBox.Show("Set of options with such name already exists." + Environment.NewLine +
                                    "Do you want to rewrite it?", LegendControl.Legend.AppName, MessageBoxButtons.YesNo, MessageBoxIcon.Question) != DialogResult.Yes) return;
                }
        ...
        private void btnRemove_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("Do you want to remove the following set of options: " + name + "?", LegendControl.Legend.AppName,
                            MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
