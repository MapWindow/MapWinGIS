...
namespace MapWindow.Legend.Forms.Categories
{
    public partial class CategoriesForm : Form
    {
        ...
        Controls.Legend.Legend m_legend;
        ...
        /// </summary>
        public CategoriesForm(Controls.Legend.Legend legend, MapWinGIS.Shapefile sf, int layerHandle)
        {
            Init(legend, sf, layerHandle);
        }

        public CategoriesForm(Controls.Legend.Legend legend, int layerHandle)
        {
            var layer = legend.Layers.ItemByHandle(layerHandle);
            if (layer != null)
            {
                var sf = layer.GetObject() as Shapefile;
                if (sf != null)
                {
                    Init(legend, sf, layerHandle);
                }
        ...
        private void Init(Controls.Legend.Legend legend, MapWinGIS.Shapefile sf, int layerHandle)
        {
            InitializeComponent();
            m_shapefile = sf;
            m_legend = legend;
            ...
            ShpfileType type = Globals.ShapefileType2D(sf.ShapefileType);
        ...
        private void btnGenerate_Click(object sender, EventArgs e)
        {
            ...
            {
                form = new frmGenerateLabelCategories(m_legend, m_shapefile, m_layerHandle);
            }
        ...
        private void btnCategoriesRemove_Click(object sender, EventArgs e)
        {
            ...
            if (dgv.CurrentCell != null)
            {
                ...
                if (tabControl1.SelectedIndex == 0)
                {
                    m_shapefile.Categories.Remove(index);
                }
                else
                {
        ...
        void UpdateGridCategory(int index, bool labels)
        {
            ...
            else
            {
                ...
                MapWinGIS.ShapefileCategory cat = m_shapefile.Categories.get_Item(index);
                dgvCategories[CMN_VISIBLE, index].Value = cat.DrawingOptions.Visible;
        ...
        private void RefreshCategoriesList(DataGridView dgv)
        {
            ...
            numCategories = categories ? m_shapefile.Categories.Count : m_shapefile.Labels.NumCategories;
        ...
        private void dgvCategories_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (dgv != null)
            {
                ...
                if (e.ColumnIndex == CMN_VISIBLE)
                {
                    ...
                    if (dgv == dgvCategories)
                    {
                        m_shapefile.Categories.get_Item(index).DrawingOptions.Visible = visible;
                    }
                    else
                    {
                ...
                if (dgv.CurrentCell != null)
                {
                    ...
                    if (dgv == dgvCategories)
                    {
                        ...
                        if (category != null)
                        {
                            ...
                            {
                                category.DrawingOptions.FillColor = Colors.ColorToUInteger(clpPointFill.Color);
                                category.DrawingOptions.PointSize = (float)udPointSize.Value;
                            }
                            else if (type == ShpfileType.SHP_POLYLINE)
                            {
                                category.DrawingOptions.LineColor = Colors.ColorToUInteger(clpLine.Color);
                                category.DrawingOptions.LineWidth = (float)icbLineWidth.SelectedIndex;
                            }
                            else if (type == ShpfileType.SHP_POLYGON)
                            {
                                category.DrawingOptions.FillColor = Colors.ColorToUInteger(clpPolygonFill.Color);
                                icbFillStyle.Color1 = clpPolygonFill.Color;
                                if (icbFillStyle.SelectedIndex == 0 && category.DrawingOptions.FillType == tkFillType.ftHatch)
                                {
                                    category.DrawingOptions.FillType = tkFillType.ftStandard;
                                }
                                else
                                {
                                    category.DrawingOptions.FillType = tkFillType.ftHatch;
                                    category.DrawingOptions.FillHatchStyle = (tkGDIPlusHatchStyle)icbFillStyle.SelectedIndex - 1;
                                }
                            }
                        }
                    }
                    else
                    {
                        ...
                        category.FontColor = Colors.ColorToUInteger(clpFont.Color);
                        category.FrameBackColor = Colors.ColorToUInteger(clpFrame.Color);
        ...
        void UpdateCategoryOptions(int index)
        {
            ...
            if (category != null)
            {
                ...
                if (type == ShpfileType.SHP_POINT || type == ShpfileType.SHP_MULTIPOINT)
                {
                    clpPointFill.Color = Colors.UintToColor(category.DrawingOptions.FillColor);
                    udPointSize.SetValue(category.DrawingOptions.PointSize);
                }
                else if (type == ShpfileType.SHP_POLYLINE)
                {
                    clpLine.Color = Colors.UintToColor(category.DrawingOptions.LineColor);
                    icbLineWidth.SelectedIndex = (int)category.DrawingOptions.LineWidth;
                }
                else if (type == ShpfileType.SHP_POLYGON)
                {
                    clpPolygonFill.Color = Colors.UintToColor(category.DrawingOptions.FillColor);
                    icbFillStyle.Color1 = clpPolygonFill.Color;

                    bool canChangeColor = category.DrawingOptions.FillType != tkFillType.ftPicture;
                    icbFillStyle.Enabled = canChangeColor;
                    clpPolygonFill.Enabled = canChangeColor;

                    if (category.DrawingOptions.FillType != tkFillType.ftHatch)
                    {
                        icbFillStyle.SelectedIndex = 0;
                    }
                    else
                    {
                        icbFillStyle.SelectedIndex = (int)category.DrawingOptions.FillHatchStyle + 1;
                    }
        ...
        void UpdateLabelOptions(int index)
        {
            ...
            if (category != null)
            {
                ...
                clpFont.Color = Colors.UintToColor(category.FontColor);
                clpFrame.Color = Colors.UintToColor(category.FrameBackColor);
        ...
        private void btnCopy_Click(object sender, EventArgs e)
        {
            ...
            {
                Globals.MessageBoxInformation("No categories to copy");
                return;
            }

            if (dgvCategories.Rows.Count  > 0)
            {
                if (MessageBox.Show("Do you want to copy categories definitions from labels?", Legend.Controls.Legend.Legend.AppName,
        ...
        private void btnLabelsCopy_Click(object sender, EventArgs e)
        {
            ...
            if (dgvLabels.Rows.Count > 0)
            {
                if (MessageBox.Show("Do you want to copy categories definitions from shapefile?", Legend.Controls.Legend.Legend.AppName,
                                MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.No)
                {
                    return;
                }
            ...
            {
                ShapefileCategory cat = categories.get_Item(i);
                if (cat != null)
                {
        ...
        public void SaveToXML()
        {
            ...
            {
                Layer layer = m_legend.GetLayer(m_layerHandle);
                if (layer != null)
                {
                ...
                //{
                //    Globals.MessageBoxError("Failed to save options: " + ex.Message);
                //}
            }
            dlg.Dispose();
        }
