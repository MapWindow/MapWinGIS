...
#
# This script expects GDAL2+ binaries
#
...
$zipCount = (Get-ChildItem -Filter release-1916-gdal-2-*.zip).count
if ($zipCount -lt 2) {
...
$zipLibsCount = (Get-ChildItem -Filter release-1916-gdal-2-*-libs.zip).count
...
Get-ChildItem -Filter release-1916-gdal-2-*.zip | Expand-Archive -DestinationPath temp\
...
$zipCount = (Get-ChildItem -Filter release-1916-x64-gdal-2-*.zip).count
if ($zipCount -lt 2) {
...
$zipLibsCount = (Get-ChildItem -Filter release-1916-x64-gdal-2-*-libs.zip).count
if ($zipLibsCount -ne 1) {
...
Get-ChildItem -Filter release-1916-x64-gdal-2-*.zip | Expand-Archive -DestinationPath temp\
