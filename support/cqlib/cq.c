...
float* m2;
{
	...
	for (r = 1; r <= 32; ++r) {
		...
		{
			area2[i] = (float)0;
			area[i] = area_r[i] = area_g[i] = area_b[i] = (long)0;
		}
		for (g = 1; g <= 32; ++g) {
			line2 = (float)0;
	...
	*cut = -1;
	for (i = first; i < last; ++i) {
		...
		if (half_w == 0) {      /* subbox could be empty of pixels! */
			...
			temp += ((float)half_r * (float)half_r + (float)half_g * (float)half_g +
				(float)half_b * (float)half_b) / (float)half_w;
...
void SetToZero()
{
	...
	for (i = 0; i < 33; ++i)
		for (j = 0; j < 33; ++j)
			for (k = 0; k < 33; ++k)
				wt[i][j][k] = mr[i][j][k] = mg[i][j][k] = mb[i][j][k] = 0;
...
/* the space for array m2 can be freed now */
	...
	unsigned char* tag = (unsigned char*)malloc(33 * 33 * 33);
	if (tag == NULL) {/*printf("Not enough space\n");*/ exit(1); }
	for (k = 0; k < K; ++k) {
		Mark(&cube[k], k, tag);
		const register long int weight = Vol(&cube[k], wt);
		if (weight) {
